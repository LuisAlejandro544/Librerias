...
try {
  ...
  for (const file of files) {
    ...
    if (fs.existsSync(filePath)) {
      ...
      // Obtener el subdirectorio para crear la carpeta si es necesario
...
`;
  ...
  fs.writeFileSync(path.join(publicDir, "floatlayer-install.sh"), installerScript, "utf-8");
  console.log("✅ Estático 'public/floatlayer-install.sh' generado correctamente.");
} catch (error) {
  console.error("❌ Error al generar 'public/floatlayer-install.sh':", error);
  process.exit(1);
}
